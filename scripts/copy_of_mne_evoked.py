...
def _line_plot_onselect(xmin, xmax, ch_types, info, data, times, text=None,
                        psd=False, time_unit='s', stc = None):
    ...
    """Draw source estimates from the selected area."""
    if stc is not None:
        import mayavi as mlab
        from ._3d import plot_source_estimates, plot_vector_source_estimates,plot_volume_source_estimates

        stc_mean = stc.copy().crop(xmin,xmax)
        stc_mean = stc_mean.mean()
        surfer_kwargs = dict(hemi='both', views='caudal',initial_time= xmin, time_unit='s', size=(500, 500), smoothing_steps=5)
        plot_source_estimates(stc_mean, **surfer_kwargs)
    else:
        print('No STC file was given.')
        print('Xmin: ', xmin)
        print('Xmax: ', xmax)
...
#         stc_mean = stc.copy()
#         print(stc_mean)
#         stc_mean = stc_mean.crop(xmin,xmax)
#         print(stc_mean)
#         stc_mean = stc_mean.mean()
#         print(stc_mean.data.shape)
#         print(stc_mean)
#         plot_source_estimates(stc_mean, **surfer_kwargs)
#     else:
#         print('No STC file was given.')
...
def _plot_evoked(evoked, picks, exclude, unit, show, ylim, proj, xlim, hline,
                 ...
                 show_names=False, group_by=None, stc=None): ### changed for source loc
    print('_plot_evoked called')
    if stc is not None:
        print('Stc is here')
    ...
    if plot_type == 'butterfly':
        _plot_lines(evoked.data, info, picks, fig, axes, spatial_colors, unit,
                    ...
                    time_unit=time_unit, stc=stc) ###changed for source loc
        print('butterfly called')
        if stc is not None:
            print(stc)
...
def _plot_lines(data, info, picks, fig, axes, spatial_colors, unit, units,
                ...
                line_alpha, nave, time_unit='ms', stc=None):
    ...
    if selectable:
        for ax in np.array(axes)[selectables]:
            ...
            callback_onselect = partial(_line_plot_onselect,
                                        ...
                                        psd=psd, time_unit=time_unit, stc=stc) ### changed for source loc
...
def plot_evoked(evoked, picks=None, exclude='bads', unit=True, show=True,
                ...
                selectable=True, noise_cov=None, time_unit='s', verbose=None, stc=None):###changed for source loc
